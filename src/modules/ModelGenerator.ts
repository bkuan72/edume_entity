        ...
        schema.forEach((prop) => {
            if (prop.fieldName !== 'id' &&
                ...
                !this.excludeFromDTO(prop, excludeProps)) {
                  const fieldName = CommonFn.toCamelCase(prop.fieldName);
 ...
 */
    ...
    public validateInsertDTOSchema({ schema, requestDTO }: { schema: schemaIfc[]; requestDTO: any; }): string | undefined {
        let error: string | undefined = '';
        let errorMsg: string | undefined = undefined;
        for (const prop in requestDTO) {
            if (this.normalSchemaField(prop)) {
                const fieldName = CommonFn.toSnakeCase(prop);
        ...
        schema.forEach((colProp) => {
            const prop = CommonFn.toCamelCase(colProp.fieldName);
    ...
    private validateProperty(colProp: schemaIfc, 
        ...
        if (colProp.fieldName === 'INDEX') {
            ...
            if (CommonFn.isString(postDTO[prop])) {
                if (colProp.trim) {
                    postDTO[prop].trim();
                }
