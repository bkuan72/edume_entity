 ...
 */
  ...
  static formatInsert = (
    ...
    return new Promise((resolve) => {
      ...
      schema.forEach(async (prop: schemaIfc) => {
        const objProp = CommonFn.toCamelCase(prop.fieldName);
      ...
      schema.forEach(async (prop: schemaIfc) => {
        let objProp = CommonFn.toCamelCase(prop.fieldName);
        if (CommonFn.hasProperty(obj, objProp) ||
            prop.fieldName === 'created_date' ||
            prop.fieldName === 'created_by' ||
            prop.fieldName === 'when_updated_usec' ||
          ...
          promiseChain = promiseChain
            ...
            .then(() => {
              ...
              if (cnt >= fldCnt) {
                ...
                first = true;
                schema.forEach(async (prop: schemaIfc) => {
                  objProp = CommonFn.toCamelCase(prop.fieldName);
                  if (CommonFn.hasProperty(obj, objProp)) {
                    if (first) {
                      first = false;
                    } else {
        ...
        if (
          ...
          if (
            ...
            data = DTOGenerator.defineProperty(
              data,
              CommonFn.toCamelCase(prop.fieldName),
              propValue
            );
            idx++;
          }
  ...
  static formatUpdate = (
    ...
    return new Promise((resolve) => {
      let propCnt = 0;
      schema.forEach(async (prop: schemaIfc) => {
        if (prop != undefined && prop.fieldName != 'id') {
          if (
            prop.fieldName === 'last_modified_date' ||
