...
class Database {
   ...
   */
  DBM_newDBSession = (): Promise<Session> => {
      ...
      if (this.DBSession == undefined) {
        ...
        mysqlx
          ...
          .then((session) => {
            ...
            SysLog.info('Connected!');
            this.DBM_initializeDatabase(dbName)
              .then(() => {
      ...
      this.DBSession.sql('SHOW DATABASES LIKE ' + CommonFn.strWrapper(dbName) + ';')
        ...
        .then((result) => {
          ...
          if (result.rows.length === 0) {
            this.DBM_createDb(dbName)
              .then(() => {
                this.DBM_selectDatabase(dbName)
                  .then(() => {
  ...
  DBM_tableExistCheck = (
    ...
    return new Promise((resolve, reject) => {
      let exist = false;
      this.DBSession.sql('SHOW TABLES LIKE ' + CommonFn.strWrapper(table.name) + ';')
        .execute()
        .then((result) => {
          if (result.rows.length === 0) {
  ...
  DBM_createDb = (dbName: string): Promise<any> => {
    return new Promise((resolve, reject) => {
      this.DBSession.sql('CREATE DATABASE ' + CommonFn.strWrapper(dbName) + ';')
        .execute()
        .then((result) => {
